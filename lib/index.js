 ...
 *   found.
 */
exports.get = ({alias, packageName, type}) => {
 ...
 * @returns {PackageInfo[]} The package information.
 */
exports.findType = ({type}) => {
  assert.string(type, 'options.type');

  return clone(packages.filter(p => p.type === type));
};
 ...
 *
 * @returns {PackageInfo[]} The package information.
 * @throws {BedrockError} Will throw a DuplicateError if the package is already
 *   registered.
 */
exports.register = ({alias, meta = {}, packageName, type}) => {
  assert.optionalString(alias, 'options.alias');
  ...
  packages.push({alias, meta, packageName, type});
};
